// generated on 2015-06-28 using generator-gulp-webapp 1.0.2
'use strict';
...
_gulp2['default'].task('views', function () {
  return _gulp2['default'].src('app/*.jade').pipe($.jade({ pretty: true })).pipe(_gulp2['default'].dest('.tmp')).pipe(reload({ stream: true }));
});
...
_gulp2['default'].task('html', ['views', 'styles'], function () {
  ...
  return _gulp2['default'].src(['app/*.html', '.tmp/*.html']).pipe(assets).pipe($['if']('*.js', $.uglify())).pipe($['if']('*.css', $.minifyCss({ compatibility: '*' }))).pipe(assets.restore()).pipe($.useref()).pipe($['if']('*.html', $.minifyHtml({ conditionals: true, loose: true }))).pipe(_gulp2['default'].dest('dist'));
});
...
_gulp2['default'].task('extras', function () {
  return _gulp2['default'].src(['app/*.*', '!app/*.html', '!app/*.jade'], {
    dot: true
...
_gulp2['default'].task('serve', ['views', 'styles', 'fonts'], function () {
